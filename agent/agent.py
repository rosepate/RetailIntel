...
from Models.forecasting import get_sales_forecast, product_location_sequences
from Models.anomaly import load_data as load_anomaly_data, detect_z_score_anomalies
...
def load_agent():
    ...
    # Load dataset
    df = pd.read_csv("C:\\Users\\rozyp\\OneDrive\\Desktop\\MLFPro\\Retailintel\\RetailIntel\\synthetic_retail_sales.csv")
...
# Reorder suggestion logic now expects a DataFrame (from file upload)
...
if __name__ == "__main__":
    ...
    while True:
        if df is None:
            file_path = input("Enter path to your CSV dataset (or 'exit' to quit): ")
            ...
            try:
                ...
                df = None
                continue
        user_query = input("Ask your question (type 'exit' to quit, 'reload' to upload new dataset): ")
        if user_query.lower() == "exit":
            break
        if user_query.lower() == "reload":
            df = None
            continue
        # Example: pass df to your agent_respond or reorder_suggestion as needed
        answer = agent_respond(user_query)  # Update this if agent_respond needs df
        print(answer)
